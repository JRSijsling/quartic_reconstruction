 ...
 *
 *  Copyright 2012-2015, R. Basson, R. Lercier, C. Ritzenthaler & J. Sijsling
 */

/***
 *
 *
 * This library aims at reconstructing over perfect fields models
 * of genus 3 hyperelliptic curves from invariants for binary forms
 * of degree 8. Given a hyperelliptic curve, we provide functions to
 * compute the 9 weighted projective invariants introduced by Shioda
 * [Shioda1967], denoted J2, J3, ..., J10, and other invariants in
 * characteristic 2, 3, 5 and 7 [Basson2015]. And conversely, given such
 * invariants, it is possible to obtain  models and the geometric automorphism
 * group (except in characteristic 5 where there are too many invariants).
 *
 * Computing the twists of any genus 3 hyperelliptic curve defined over a finite
 * field (except in characteristic 5), especially for curves with many
 * automorphisms, is implemented too.
 *
 * Details about the algorithms implemented in this package can be found in
 * [LeBa2016,Basson2015,LeJeRi2015,LRS2013,LeRi2012].
 *
 *
 * Bibliography
 *
 * [LeBa2016] "Invariants for hyperelliptic curves of genus 3 in
 * characteristic 2", R. Basson and R. Lercier. Preprint.
 *
 * [Basson2015] "Arithmétique des espaces de modules des courbes
 * hyperelliptiques de genre 3 en caractéristique positive", R. Basson,
 * PHD thesis, Université de Rennes 1, 2015.
 * Available at https://tel.archives-ouvertes.fr/tel-01170922
 *
 * [LeJeRi2015] "Explicit Galois Obstruction and Descent for Hyperelliptic
 * Curves with Tamely Cyclic Reduced Automorphism Group", R. Lercier,
 * C. Ritzenthaler, and J. Sijsling. Mathematics of Computation, 2015.
 * To appear.
 *
 * [LRS2013] "Fast computation of isomorphisms of hyperelliptic curves and
 * explicit descent", R. Lercier, C. Ritzenthaler, and J. Sijsling.
 * Proceedings of the Tenth Algorithmic  Number Theory Symposium,
 * volume 1 of The Open Book Series, pages 463-486.
 ...
 * Changes.
 *
 * 2016-01-25, v2.0 : R. Lercier
 *             Twists in fields of characteristic 2
 *
 * 2015-04-13, v1.2 : R. Basson, R. Lercier, C. Ritzenthaler
 *             Fields of characteristic 2, 3 and 7
 *
 * 2011-12-27, v1.1 : R. Lercier, C. Ritzenthaler,
 *             Descent over the field of moduli in all cases.
 *
...
intrinsic ShiodaInvariants(f::RngUPolElt : normalize := false, scaled := true) -> SeqEnum, SeqEnum
    ...
    require not Characteristic(CR) in {2} : "2 must be invertible in the base ring.";

    /* Rings of small characteristic  */
    case Characteristic(CR):

    when 3:
	JI, Wght := ShiodaInvariantsChar3(f : normalize := normalize);
	return JI, Wght;

    when 5:
	JI, Wght := ShiodaInvariantsChar5(f : normalize := normalize);
	return JI, Wght;

    when 7:
	JI, Wght := ShiodaInvariantsChar7(f : normalize := normalize);
	return JI, Wght;
     ...
     * Intermediary covariants */
      ...
      * Fundamental covariants */
      ...
      if normalize eq false then return JI, [2..10]; end if;
      return WPSNormalize([2..10], JI), [2..10];
...
function ShiodaAlgebraicInvariantsChar0(FreeJI, ratsolve)
...
intrinsic ShiodaAlgebraicInvariants(FreeJI::SeqEnum : ratsolve := true) -> SeqEnum
    ...
    return ShiodaAlgebraicInvariantsChar0(FreeJI, ratsolve);
...
function IJInvariants(K)
    E, D, C, B, A := Explode([Coefficient(K, i) : i in [0..4]]);
    I := 12*A*E - 3*B*D + C^2;
...
function G3ModelsInCharFF_G8_5(JI : geometric := false)
    ...
    */
    ...
    /* The difficult case, let us use he covariant method to descend this curve */
    if not IsFinite(FF) then

	if Type(FF) eq FldRat or IsNumberField(FF) then K3 := ext<FF | Pa4>; else K3 := quo<Parent(x) | Pa4>; end if;
	a4 := K3.1; x := PolynomialRing(K3).1;
	la8 := (18*J6*a4-9*a4*J4*J2-60*a4*J3^2+2*a4*J2^3-810*J7+270*J5*J2-810*J4*J3)/(-18*J6+9*J4*J2+60*J3^2-2*J2^3)/10;
	a6 := (-28*la8^2-1/5*a4^2+14*J2);
	l := 1/a6; a8 := a6*la8;

	f1 := a8*x^8 + a6*x^6 + a4*x^4 + l*a6*x^2 + l^2*a8;

	/* Let us determine a non trivial transvectant */
	f1_cov := [* f1, 1, 8 *];

	k_cov := Transvectant(f1_cov, f1_cov, 6);
	I, J := IJInvariants(k_cov[1]);

	if 4*I^3-J^2 eq 0 or I eq 0 or J eq 0 then
	    g_cov := Transvectant(f1_cov, f1_cov, 4);
	    k_cov := Transvectant(f1_cov, f1_cov, 6);

	    I, J := IJInvariants(k_cov[1]);

	    if 4*I^3-J^2 eq 0 or I eq 0 or J eq 0 then
		k_cov := Transvectant(k_cov, f1_cov, 4);
		I, J := IJInvariants(k_cov[1]);
	    end if;
	end if;

	if Type(K3) eq FldRat or IsNumberField(K3) then KC := ext<K3 | k_cov[1]>; else KC := quo<Parent(x) | k_cov[1]>; end if;
...
intrinsic HyperellipticCurveFromShiodaInvariants(JI::SeqEnum: RationalModel := true, Deterministic := false) -> CrvHyp, GrpPerm
    ...
    "HyperellipticPolynomialsFromShiodaInvariants".}
...
intrinsic TwistedHyperellipticPolynomialsFromShiodaInvariants(JI::SeqEnum[FldFinElt] : RationalModel := true, Deterministic := false) -> SeqEnum[CrvHyp], GrpPerm
    ...
    require Type(FF) eq FldFin : "Twist computations only available in finite fields.";
...
intrinsic GeometricAutomorphismGroupClassificationForGenus3HyperellipticCurve(FF::FldFin) -> SeqEnum, SeqEnum
    ...
    when 7:
	G672:= 1;
	C2C4 := q-2; D6 := q-2; C2D4 := q-2;
	C4  := q^2-2*q+2; C2p3 := q^2-2*q+2;//Unproved
	D2 := q^3-2*q^2+2; // Unproved
	C2 := q^5-q^3+q-1; // Unproved
	...
	if D6 ne 0 then
	    Grps cat:= [*DihedralGroup(6)*]; Nmbs cat:= [D6];
	end if;
	if C2D4 ne 0 then
	    Grps cat:= [*DirectProduct(CyclicGroup(2), DihedralGroup(4))*]; Nmbs cat:= [C2D4];
	end if;
	if C4  ne 0 then
